...
public class ChatClient extends JFrame implements KeyListener, TcpNodeListener
{
    ...
    connectBtn.addActionListener(new ActionListener()
    {
      ...
      public void actionPerformed(ActionEvent evt)
      {
        if (tcpNode == null)
        {
          ...
          tcpNode.connect(id, nickname, 1, 1, 1);
        }
        else
        {
          tcpNode.disconnect();
  ...
  {
    if (evt.getKeyCode() == KeyEvent.VK_ENTER) // enter -> sende daten
    {
      tcpNode.sendMessage(line);
      messageReceived(nickname, line);
      line = "";
      outputArea.setText(""); // setzt "eingabefenster" zurück
      evt.consume(); // zerstört event (sonst leere Zeile)
    }
    else
    {
      char ch;
      if (evt.getKeyCode() == 8 && line.length() > 0) // backspace
